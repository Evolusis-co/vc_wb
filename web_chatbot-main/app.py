...
from models import User, Company, RevokedToken, ChatSession, ChatMessage
...
def initialize_services():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Database init failed (non-critical): {db_error}")
...
# Optional: use voiceCoach auth service when provided, otherwise fall back to local validators
from utils.auth_validators import EmailValidator, PasswordValidator, NameValidator, sanitize_input


def forward_to_auth_service(path: str, payload: dict) -> (bool, dict):
    """Forward auth-related requests to an external auth service (voiceCoach)."""
    if not AUTH_SERVICE_URL:
        return False, {"error": "No auth service configured"}

    url = AUTH_SERVICE_URL.rstrip('/') + path
    try:
        ...
        logger.error(f"Failed to forward to auth service: {e}")
        return False, {"error": str(e)}


# ---------- Auth endpoints (signup/login/status) ----------
@app.route('/auth/signup', methods=['POST'])
def auth_signup():
    """Signup endpoint: validates inputs and creates user in database"""
    db = get_db_session()
    try:
        ...
        NameValidator.validate_or_raise(name)
        ...
        logger.info(f"✅ New user registered: {sanitized_email}")
...
def auth_logout():
    ...
    try:
        ...
        if not token:
            return jsonify({'message': 'Not logged in', 'success': True})

        # Decode token to get JTI
        try:
            payload = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM], options={"verify_exp": False})
            jti = payload.get('jti', token[:64])  # Use token hash if no JTI
        ...
        return jsonify({'message': 'Logout completed', 'success': True})
    finally:
        db.close()
